...
describe('#addUserToTeamTransaction', () => {
  ...
  test('Should add user to team in transaction and user should be given expected team scopes', async () => {
    const { db } = request
    const teamId = platformTeamFixture._id
    const userId = userTenantWithoutTeamFixture._id

    await replaceOneTestHelper(teamCollectionName, platformTeamFixture)
    await replaceManyTestHelper(scopeCollectionName, [
      adminScopeFixture,
      externalTestScopeFixture
    ])
  ...
  test('Should be idempotent for the same user and team pair', async () => {
    ...
    await replaceOneTestHelper(teamCollectionName, tenantTeamFixture)
    await replaceOneTestHelper(scopeCollectionName, postgresScopeFixture)
    await replaceOneTestHelper(userCollectionName, userTenantWithoutTeamFixture)
  ...
  test('Should add team membership when team has no scopes', async () => {
    ...
    await replaceOneTestHelper(teamCollectionName, teamWithoutUsers)
    await replaceOneTestHelper(userCollectionName, userAdminOtherFixture)
