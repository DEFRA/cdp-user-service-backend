...
describe('#deleteTeamTransaction', () => {
  beforeAll(async () => {
    vi.useFakeTimers()
  ...
  test('Should delete team, team scopes and remove members', async () => {
    const { db } = request
    ...
    // Now delete the team
    await deleteTeamTransaction({
      request,
      teamId
    })

    expect(mockInfoLogger).toHaveBeenCalledWith(
      `Team ${teamName} deleted from CDP`
    )
    ...
    // Check team has been removed from the user
    const updatedUser = await db
      .collection(userCollectionName)
      .findOne({ _id: userId })
    expect(updatedUser.teams).toEqual([])

    // Check that all scopes have had the team removed
  ...
  test('Should rollback when a write fails within transaction', async () => {
    ...
    // Check team, user and all scopes are unchanged
  ...
  test('Should throw error when team does not exist in the db', async () => {
    await expect(
      deleteTeamTransaction({
        request,
