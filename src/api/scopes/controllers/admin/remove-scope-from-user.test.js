...
describe('#/scopes/admin/{scopeId}/user/remove/{userId}', () => {
  let server
  let db
  ...
  it('should remove a permission from a user', async () => {
    ...
    const remainingScopeId = scopeDefinitions.restrictedTechPython.scopeId

    await grantPermissionToUser(db, user1._id, scopeId)
    await grantPermissionToUser(db, user1._id, remainingScopeId)

    const { statusCode } = await callWithAuth(
      `/scopes/admin/${scopeId}/user/remove/${user1._id}`,
      'PATCH'
    )
  ...
  it('should 400 when applied to a invalid scope', async () => {
    const { statusCode } = await callWithAuth(
      `/scopes/admin/invalid/user/remove/${user1._id}`,
      'PATCH'
    )
